    ...
    >
      ...
      <div className="bg-slate-950/80 border border-slate-800/90 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-mono">
        <div>
          ...
          <div className="text-xs text-slate-400 flex flex-wrap items-center gap-2 mt-1">
            ...
            <span>·</span>
            <span className="text-cyan-300 font-bold">n={lifetime.resolvedSignals} resolved</span>
            <span>·</span>
            <span className="text-slate-400">95% Wilson CI: [{calibrationMetrics.wilsonIntervalLowerPct}% – {calibrationMetrics.wilsonIntervalUpperPct}%]</span>
          </div>
        </div>
        ...
        <div className="bg-slate-900/90 border border-amber-500/30 px-3.5 py-2.5 rounded-xl text-left sm:text-right max-w-sm">
          <div className="flex items-center sm:justify-end space-x-1.5 text-amber-400 text-[10px] font-bold uppercase">
            ...
            <span>Calibration: {calibrationMetrics.calibrationStatus.replace('_', ' ')}</span>
          </div>
          <p className="text-[10px] text-slate-400 font-sans mt-0.5 leading-tight">
            Observed {lifetime.winRate}% is an empirical result (n={lifetime.resolvedSignals}). Asymptotic reliability calibration requires n &gt; 50 samples.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 font-mono text-xs">
        ...
        <div className="p-3 bg-slate-950/70 border border-rose-500/30 rounded-xl">
          ...
          <span className="text-[10px] text-slate-500 block mt-0.5">{lifetime.losses} Completed Losses</span>
        </div>

        {/* Metric 3: TOPSIS Vetted Score (Decision/Ranking Metric) */}
        <div className="p-3 bg-slate-950/70 border border-cyan-500/30 rounded-xl">
          ...
          <span className="text-[9px] text-slate-400 block mt-0.5">Ranking Metric (A+)</span>
        </div>

        {/* Metric 4: AHP Truth Consensus (Consensus Metric) */}
        <div className="p-3 bg-slate-950/70 border border-indigo-500/30 rounded-xl">
          ...
          <span className="text-[9px] text-slate-400 block mt-0.5">Neutrosophic Consensus</span>
        </div>

        {/* Metric 5: Mean Calibrated Prediction Probability & Calibration Error */}
        <div className="p-3 bg-slate-950/70 border border-emerald-500/40 rounded-xl col-span-2 sm:col-span-1">
          ...
          <span className="text-[9px] text-cyan-400 font-semibold block mt-0.5">
            ECE: {calibrationMetrics.expectedCalibrationError ?? 0}% | Brier: {calibrationMetrics.brierScore ?? 0}
          </span>
        </div>
      </div>
...
// Also export as AccuracyMetricsWidget for backward compatibility
export const AccuracyMetricsWidget = AccuracyMetrics;
